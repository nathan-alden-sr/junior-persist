...
namespace Junior.Persist.Persistence.SqlServer.Repositories
{
	/// <summary>
		...
		where TDataConnector : class, IInsertingOrUpdatingDataConnector<TEntityData>
	{
		...
		public IEnumerable<CacheEntity<TEntity>> Persist(
			...
			Func<Guid, IEnumerable<Guid>> getExistingEntityIdsDelegate,
			...
			Action<Guid> beforeEntityRemovedDelegate = null,
			Action<TEntity> afterEntityUpdatedDelegate = null)
		{
			...
			var synchronizer = new EnumerableSynchronizer<Guid, TEntity>(getExistingEntityIdsDelegate(parentEntityId), entities);
			...
			Action<Guid> removedItemDelegate = arg =>
			                                   	{
